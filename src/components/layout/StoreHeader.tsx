...
export function StoreHeader({ overlay = false }: { overlay?: boolean }) {
  ...
  const navLinkClasses = cn("block cursor-pointer rounded-full px-3 py-2 text-base font-normal leading-6 aria-[current=page]:underline aria-[current=page]:underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current focus-visible:ring-offset-2", isTransparent ? "hover:bg-white/14" : "hover:bg-surface-subtle");
  ...
  return (
    <header className={cn("fixed inset-x-0 top-0 z-50 border-b transition-[background-color,border-color,box-shadow] duration-200", isTransparent ? "border-transparent bg-transparent text-white" : "border-border/80 bg-background/95 text-primary shadow-[0_8px_30px_rgba(9,35,40,0.06)] backdrop-blur-md")}>
      <div className="mx-auto flex h-18 w-full max-w-[90rem] items-center justify-between px-4 sm:px-6 lg:h-20 lg:px-8 xl:px-12">
        <Link href="/" aria-label="KOMO — Trang chủ" className="cursor-pointer rounded-sm focus-visible:ring-2 focus-visible:ring-current"><span className="font-heading text-2xl font-semibold tracking-[-0.05em] sm:text-3xl">KOMO</span></Link>
        <div className="hidden items-center gap-1 xl:flex">
          <nav aria-label="Điều hướng chính"><ul className="flex items-center gap-0.5">
            <li><Link href="/" aria-current={pathname === "/" ? "page" : undefined} className={cn(navLinkClasses, "font-semibold")}>Trang chủ</Link></li>
            <li ref={productsRef} className="relative" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setIsProductsOpen(false); }}>
              <button ref={productsButtonRef} type="button" className={cn(navLinkClasses, "flex items-center gap-1.5")} aria-expanded={isProductsOpen} aria-controls="desktop-products-menu" onClick={() => setIsProductsOpen((open) => !open)} onKeyDown={(event) => { if (!isProductsOpen && ["ArrowDown", "Enter", " "].includes(event.key)) { event.preventDefault(); openProducts(); } }}>Sản phẩm <ChevronIcon open={isProductsOpen} /></button>
              <div id="desktop-products-menu" className={cn("absolute left-0 top-[calc(100%+0.5rem)] w-56 rounded-card border border-border bg-surface p-2 text-primary shadow-[0_16px_40px_rgba(9,35,40,0.14)]", isProductsOpen ? "block" : "hidden")}>{productNavigation.map((item, index) => <Link key={item.href} ref={index === 0 ? firstProductRef : undefined} href={item.href} aria-current={pathname === item.href ? "page" : undefined} className="flex min-h-11 cursor-pointer items-center rounded-button px-4 py-2 text-base font-normal leading-6 hover:bg-surface-subtle aria-[current=page]:bg-surface-subtle" onClick={() => setIsProductsOpen(false)}>{item.label}</Link>)}</div>
            </li>
            <li><Link href="/about" aria-current={pathname === "/about" ? "page" : undefined} className={navLinkClasses}>Về chúng tôi</Link></li>
          </ul></nav>
          {isSearchOpen ? <SearchForm inputId="header-search-desktop" inputRef={searchInputRef} transparent={isTransparent} onClose={closeSearch} /> : <button ref={searchButtonRef} type="button" className={navLinkClasses} onClick={openSearch} aria-expanded={isSearchOpen} aria-controls="header-search-desktop">Tìm kiếm</button>}
          <Link href="/cart" aria-current={pathname === "/cart" ? "page" : undefined} className={navLinkClasses}>Giỏ hàng</Link>
          <Link href="/login" className={cn("ml-2 inline-flex min-h-11 cursor-pointer items-center rounded-button px-5 text-base font-normal leading-6 focus-visible:ring-2 focus-visible:ring-current", isTransparent ? "border border-white/55 bg-white text-primary hover:bg-background" : "bg-primary text-white hover:bg-primary-hover")}>Đăng nhập</Link>
        </div>
      ...
      <div className={cn("fixed inset-0 z-50 bg-primary/55 transition-opacity xl:hidden", isMenuOpen ? "visible opacity-100" : "invisible pointer-events-none opacity-0")} aria-hidden="true" onClick={() => setIsMenuOpen(false)} />
      <div ref={drawerRef} id="mobile-navigation" className={cn("fixed inset-y-0 right-0 z-50 flex w-[min(88vw,23rem)] max-w-full flex-col overflow-y-auto bg-background p-6 text-primary shadow-2xl transition-[transform,visibility] duration-200 xl:hidden", isMenuOpen ? "visible translate-x-0" : "invisible pointer-events-none translate-x-full")} role="dialog" aria-modal="true" aria-label="Menu điều hướng" aria-hidden={!isMenuOpen} inert={!isMenuOpen}>
        ...
        <nav aria-label="Điều hướng di động" className="mt-5"><ul className="grid gap-1">
          <li><MobileLink href="/" aria-current={pathname === "/" ? "page" : undefined} emphasized onClick={() => setIsMenuOpen(false)}>Trang chủ</MobileLink></li>
          <li><button type="button" className="flex min-h-12 w-full cursor-pointer items-center justify-between rounded-button px-4 py-3 text-base font-normal leading-6 hover:bg-surface-subtle" aria-expanded={isMobileProductsOpen} aria-controls="mobile-products-menu" onClick={() => setIsMobileProductsOpen((open) => !open)}>Sản phẩm <ChevronIcon open={isMobileProductsOpen} /></button><div id="mobile-products-menu" className={cn("ml-4 border-l border-border pl-2", isMobileProductsOpen ? "grid" : "hidden")}>{productNavigation.map((item) => <MobileLink key={item.href} href={item.href} aria-current={pathname === item.href ? "page" : undefined} onClick={() => setIsMenuOpen(false)}>{item.label}</MobileLink>)}</div></li>
          <li><MobileLink href="/about" aria-current={pathname === "/about" ? "page" : undefined} onClick={() => setIsMenuOpen(false)}>Về chúng tôi</MobileLink></li>
          <li><MobileLink href="/cart" aria-current={pathname === "/cart" ? "page" : undefined} onClick={() => setIsMenuOpen(false)}>Giỏ hàng</MobileLink></li>
        </ul></nav>
        <Link href="/login" className="mt-6 inline-flex min-h-12 cursor-pointer items-center justify-center rounded-button bg-primary px-5 text-base font-normal leading-6 text-white hover:bg-primary-hover" onClick={() => setIsMenuOpen(false)}>Đăng nhập</Link>
